...
void FE::engine::__read_froggy() noexcept
{
...

	{
		...
		auto l_resource_lut = l_project_config["GlobalResourceLookUpTable"].as_object();
		...
		*const_cast<var::uint16*>(&(m_project_config->_fibers_per_thread)) = static_cast<FE::uint16>(l_project_config["FibersPerThread"].get_int64());
...

		{
			...
			auto l_window_config = l_project_config["WindowConfig"].as_object();
			...
			*const_cast<var::boolean*>(&(m_project_config->_window_config._should_scale_content_to_monitor_dpi)) = l_window_config["ShouldScaleContentToMonitorDPI"].get_bool();
			*const_cast<var::boolean*>(&(m_project_config->_window_config._has_border)) = l_window_config["HasBorder"].get_bool();
			...
			*const_cast<var::boolean*>(&(m_project_config->_window_config._is_maximized)) = l_window_config["IsMaximized"].get_bool();
			*const_cast<var::boolean*>(&(m_project_config->_window_config._is_fullscreen)) = l_window_config["IsFullScreen"].get_bool();
		}
	}
}
...
void FE::engine::__cursor_position_callback(GLFWwindow* const window_p, double x_p, double y_p) noexcept
{
	...
	if (l_mouse._mouse_callbacks._cursor_position_callback)
	{
...
void FE::engine::__scroll_callback(GLFWwindow* const window_p, double x_offset_p, double y_offset_p) noexcept
{
	...
	if (l_mouse._mouse_callbacks._scroll_callback)
	{

...
namespace FHT::parser
{
	...
	_FE_NODISCARD_ header_file_root try_build_reflection_tree(const directory_t& file_path_p, const std::pmr::list<token>& token_list_p)
	{
		...
		for (auto iterator = token_list_p.begin(); iterator != token_list_p.end(); ++iterator)
		{
			...
			case Vocabulary::_BeginNamespace:
				...
				l_root._namespaces.push_back(try_build_namespace_node_recursive(u8"", iterator, token_list_p.end()));
				break;
			...
			case Vocabulary::_Class:
				if (is_forward_declaration(iterator) == true)
				{
					l_is_template = false;
					break;
				}
				...
				{
					skip_code_block(iterator, token_list_p.end());
					l_is_template = false;
					break;
				}

				l_root._classes.push_back(try_build_class_node_mutually_recursive(u8"", iterator, token_list_p.end()));
				break;
			...
			case Vocabulary::_Struct:
				...
				l_root._structs.push_back(try_build_struct_node_mutually_recursive(u8"", iterator, token_list_p.end()));
				break;

			case Vocabulary::_FrogmanEngineEnumStructReflectionMacro:
				THROW_CPP_SYNTAX_ERROR(l_is_template == true, "C++ code syntax Error: \nThe line number: ${%u32@0} \nCan't place 'template' before 'FE_ENUM_STRUCT().'", &(iterator->_line_number));
				l_root._enum_structs.push_back(try_build_enum_struct_node(u8"", iterator, token_list_p.end()));
				break;
			...
			{
				std::optional<system_node> l_system_function_name = try_build_c_style_system_function_node(u8"", iterator, token_list_p.end());
		...
		*/
		...
		for (; out_token_iterator_p != end_p; ++out_token_iterator_p)
		{
			...
			case Vocabulary::_Namespace:
				_FE_FALLTHROUGH_;
			case Vocabulary::_BeginNamespace:
				l_node._nested_namespaces.push_back(try_build_namespace_node_recursive(l_node._target_namespace_name, out_token_iterator_p, end_p));
				break;
			...
			case Vocabulary::_Template:
				...
				skip_template_args(out_token_iterator_p);
				if (is_forward_declaration(out_token_iterator_p) == false)
				{
					skip_code_block(out_token_iterator_p, end_p);
				}
				break;
			...
			case Vocabulary::_Class:
				if (is_forward_declaration(out_token_iterator_p) == true)
				{
					break;
				}
				l_node._classes.push_back(try_build_class_node_mutually_recursive(l_node._target_namespace_name, out_token_iterator_p, end_p));
				break;
			...
			case Vocabulary::_Struct:
				...
				l_node._structs.push_back(try_build_struct_node_mutually_recursive(l_node._target_namespace_name, out_token_iterator_p, end_p));
				break;

			case Vocabulary::_FrogmanEngineEnumStructReflectionMacro:
				l_node._enum_structs.push_back(try_build_enum_struct_node(l_node._target_namespace_name, out_token_iterator_p, end_p));
				break;
			...
			case Vocabulary::_FrogmanEngineSystemAttributeMacro:
			{
				std::optional<system_node> l_system_function_name = try_build_c_style_system_function_node(l_node._target_namespace_name, out_token_iterator_p, end_p);
				if (l_system_function_name == std::nullopt)
				{
					break;
				}
		...
		*/
		...
		case Vocabulary::_Colon:
			...
			default: // the base class name found.
				l_node._base_class_name = identifier(framework::get_framework().get_memory_resource());
		...
		l_node._this_class_name = identifier(parent_namespace_p, framework::get_framework().get_memory_resource());
	...
	Break:
		identifier l_function_declaration_buffer(framework::get_framework().get_memory_resource());
		...
		identifier l_constructor(l_class_name, framework::get_framework().get_memory_resource());
		l_constructor += u8"()";
	...
	_FE_NODISCARD_ std::optional<struct_node> try_build_struct_node_mutually_recursive(const identifier& parent_namespace_p, typename std::pmr::list<token>::const_iterator& out_token_iterator_p, typename std::pmr::list<token>::const_iterator end_p)
	{
		...
		*/
		...
		l_node._target_struct_name = identifier(parent_namespace_p, framework::get_framework().get_memory_resource());
	...
	_FE_NODISCARD_ std::optional<enum_struct_node> try_build_enum_struct_node(const identifier& parent_namespace_p, typename std::pmr::list<token>::const_iterator& out_token_iterator_p, typename std::pmr::list<token>::const_iterator end_p)
	{
		...
		if (out_token_iterator_p->_vocabulary == Vocabulary::_Enum)
		{
			...
			l_node._target_enum_struct_name = identifier(parent_namespace_p, framework::get_framework().get_memory_resource());
			...
			l_node._enum_struct_fields = std::pmr::vector<identifier>(framework::get_framework().get_memory_resource());
	...
	_FE_NODISCARD_ std::optional<system_node> try_build_c_style_system_function_node(const identifier& parent_namespace_p, typename std::pmr::list<token>::const_iterator& out_token_iterator_p, typename std::pmr::list<token>::const_iterator end_p)
	{
		...
		identifier l_system_parameter(framework::get_framework().get_memory_resource());
		...
		l_node._sysname = identifier(framework::get_framework().get_memory_resource());

		...
		m_backend(this)
{
	...
	{
		std::memcpy( const_cast<var::int32*>( &(m_window_config._width) ), &(m_video_mode->width), sizeof(FE::int32) );
	}
	if (m_window_config._height == 0)
	{
		std::memcpy( const_cast<var::int32*>( &(m_window_config._height) ), &(m_video_mode->height), sizeof(FE::int32) );
	}

	glfwWindowHint(GLFW_FLOATING, m_window_config._is_on_the_top); // Make window always on top
	glfwWindowHint(GLFW_CENTER_CURSOR, m_window_config._is_cursor_centered); // Center the cursor in the window
